...
void read_input_parameters(int, char **, char *, long *, long *);//void read_input_parameters(int, char **, char *, char *, char *, long *, long *);
...
int main (int argc, char* argv[])
{
  ...
  read_input_parameters(argc,argv,modelfile,
      &verbosity,&pred_format); //read_input_parameters(argc,argv,docfile,modelfile,predictionsfile, &verbosity,&pred_format);
...
//  doc.words = (WORD *)my_malloc(sizeof(WORD)*(max_words_doc+10));
  ...
  char buff[30];
  while((!feof(stdin)) && fgets(line,(int)lld,stdin)) {
//    if(line[0] == '#') continue;  /* line contains comments */
    //sprintf(buff,"line.legth=%lu",strlen(line));
    //perror(buff);
